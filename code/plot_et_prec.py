...
def make_plot():
    ...
    plot_map_tb(et.sum(dim='month'),ax=ax1, levels=levels1,pr=pr)
    set_lat_lon(ax1, range(75,105,10), range(25,40,10), label=True, pad=0.05, fontsize=10)
    ax1.set_title('ET',fontsize=12)

    ################### Panel b: annual Prec
    ax2 = fig.add_subplot(2, 1, 2, projection=pr)
    plot_map_tb(dp.prec.sum(dim='month'),ax=ax2, levels=np.arange(0,901,100))
    ...
    plt.savefig('../figure/figure_et_prec.png',dpi=300,bbox_inches='tight')
    print('figure saved')
